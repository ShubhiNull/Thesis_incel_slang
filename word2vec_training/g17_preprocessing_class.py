...
class Reader17:
'''
    Class to preprocess data from reddit dataset 2017
'''
    ...
    def __iter__(self):  
        for fname in os.listdir(self.directory):
            ...
            if fname.endswith(".parquet"):
                ...
                #filtering posts from subreddits which have the word pill or incel
                dataframe['filter'] = dataframe['subreddit'].str.contains(r'pill|incel', case = False)
                filtered_df = dataframe[dataframe['filter'] == False]
                for text in filtered_df['body']:
                    yield self.preprocess(text)
                    #print (filtered_df['subreddit'])    
        ...
        for sentence in list_of_sentences:
            ...
            for i, word in enumerate(token_words):
                if word and not all(chr in punct for chr in word) and not word.startswith(("//", "gif", "giphy","https")):
                    ...
                    max_match = re.match(r"^(.*?)(maxx+)(ing|ed|er|es|s)?$", word_lower)  
                    if max_match and word not in {"tj-maxx", "tk-maxx", "tjmaxx", "tkmaxx"}:
                        ...
                        continue
                    ma_match = re.match(r"^(.*?)(max)(ing|ed|er|es|s)?$", word_lower)
